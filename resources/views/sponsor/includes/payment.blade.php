            ...
            @foreach ($sponsors as $sponsor)
              <li>
                <div class="form-check">
                  <input class="form-check-input" type="radio" name="sponsor" id="sponsor-{{ $sponsor->id }}" value="{{ $sponsor->id }}"
            ...
            <div class="modal-body">
                <ul>
                    @foreach ($sponsors as $sponsor)
                        <li>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="sponsor" id="sponsor-{{ $loop->index + 1 }}" value="{{ $loop->index + 1 }}"
                                @if ($loop->first) checked @endif>
                                <label class="form-check-label" for="sponsor-{{ $loop->index + 1 }}">
                                    <span class="me-2" style="background-image: linear-gradient(to left,{{ $sponsor['color'] }});-webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; font-weight: 700;">{{ $sponsor['label'] }}</span> €{{ $sponsor['price'] }} / {{ $sponsor['duration'] }}h
                                </label>
                            </div>
                        </li>
